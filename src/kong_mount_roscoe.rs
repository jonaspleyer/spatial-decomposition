...
) -> Result<Vec<(Rectangle<F>, Vec<(I, Rectangle<F>)>)>, Box<dyn std::error::Error>>
where
    F: 'static + Copy + RealField,
    F: num_traits::cast::AsPrimitive<usize>,
    usize: num_traits::cast::AsPrimitive<F>,
    I: 'static,
{
    ...
    let mut map: std::collections::BTreeMap<usize, (Rectangle<F>, Vec<(I, Rectangle<F>)>)> =
        subdomains
            .into_iter()
            .enumerate()
            .map(|(n_subdomain, subdomain)| (n_subdomain, (subdomain, Vec::new())))
            .collect();
    for (digit, rect) in digits.into_iter() {
        let mut ret = false;
        for (_, (subdomain, values)) in map.iter_mut() {
            let middle = [
                (rect.min[0] + rect.max[0]) / (F::one() + F::one()),
                (rect.min[1] + rect.max[1]) / (F::one() + F::one()),
            ];
        ...
        if !ret {
            ...
            return Err(Box::new(MyError(
                "Could not find suitable subdomain for given digit".into(),
            )));
        }
    }
